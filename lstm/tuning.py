...
#     custom_early_stopping_instance = CustomEarlyStopping(min_delta=min_delta, patience=patience, verbose=True)
    ...
    X_valid_reshaped = X_valid.values.reshape((X_valid.shape[0], 1, X_valid.shape[1]))
    ...
    with tf.device('GPU:0'):  # Ensure the model runs on GPU
        ...
        model.fit(X_train_reshaped, y_train_flat, epochs=params['epochs'], batch_size=params['batch_size'], validation_data=(X_valid_reshaped, y_valid_flat), verbose=1)
...
data = pd.read_csv('lstm-tuning/save_data.csv')
